...
If we were using a max heap, the values in the new array are in descending order, for a min heap they would be in ascending order. When we pop a node from a max-heap it will always be the root (the largest value), which is replaced by the last node. The last node will trickle down through swapping with its largest child, as shown in the previous task. The resultant heap leaves the largest value as the new root node, and thus the cycle continues. Since our max heap logic results in the largest value always being placed in the root node we get a nice descending ordered array. 
